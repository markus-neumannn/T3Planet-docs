...
description: "Custom AI Providers for EXT:ns_t3af (T3AF)."
keywords:
  - "TYPO3"
  - "T3Planet"
  - "T3AF"
  - "ns_t3af"
sidebarTitle: "Custom AI Providers"
---
...
T3AF discovers provider adapters from the Symfony service container. Each adapter describes its type, display name, default endpoint, supported capabilities, connection test, and runtime platform object.

Built-in bridge packages are auto-registered when available. T3AF also provides the built-in **Custom / Other** OpenAI-compatible adapter under `nst3af.openai_compatible`. Use a custom adapter only when the provider is not OpenAI-compatible or needs a custom SDK/protocol.
...
- [Configuration](/T3AF/Configuration/Index)
