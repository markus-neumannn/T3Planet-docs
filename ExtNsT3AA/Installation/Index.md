...
T3AA needs **T3AF** (`EXT:ns_t3af`). T3AF connects your AI providers (API keys, models, prompts, and shared AI services). Without it, T3AA cannot run.
...
3. **T3AF** (`EXT:ns_t3af`) — shared AI engine used by T3AA
...
2. Update the `only` parameter so the project can download T3AA: "only": [
  "nitsan/ns-t3aa"
]
3. Install the T3AA package: composer require nitsan/ns-t3aa
...
T3AF (`EXT:ns_t3af`) is required before T3AA can be used.
...
If T3AF is not already present after installing T3AA, install it with:
...
- [T3AF First Setup](/ExtNsT3AF/Installation/Index)
...
T3AA will not function correctly until T3AF has a working AI provider configuration.
