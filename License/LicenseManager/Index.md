---
title: "Using License Key in T3Planet Shop"
description: "Activate a license key, bind it to your domain, and monitor license health from T3Planet Shop in the TYPO3 backend."
keywords:
  - "TYPO3"
  - "T3Planet"
  - "License"
  - "Using License Key in T3Planet Shop"
  - "LicenseManager"
  - "T3Planet Shop"
sidebarTitle: "Using License Key"
---

## Purpose

Activate a license key, bind it to your domain, and monitor license health from T3Planet Shop in the TYPO3 backend.

## Demo

<div className="t3-embed"><iframe src="https://app.supademo.com/embed/cmn4aw73y0ooaz3qm7owt0691?embed_v=2&utm_source=embed&step=1" loading="lazy" title="Activate and Manage Extension Licenses with Statistics" allow="clipboard-write; fullscreen" frameBorder="0" webkitallowfullscreen="true" mozallowfullscreen="true" allowfullscreen></iframe></div>

## Steps

1. Open TYPO3 Backend and go to `[T3Planet Shop]`.
2. Paste your key into the `[License Key]` field (or start a trial / purchase via `[Get New License]`).
3. Click `[Activate License & Download Extension]` to start activation.
4. Confirm domain binding for the current TYPO3 instance when prompted.
5. Open the dashboard statistics area to review license insights.

## License Statistics View

You can typically review:

- Active install/license status
- Expiry and renewal information
- Usage/download-related statistics
- Extension metadata, such as rating/documentation links

<Note>
If activation fails, verify that the registered domain matches the current TYPO3 instance URL and that the key has not expired.
</Note>

## Managing Multiple Licenses

1. Stay on the same T3Planet Shop dashboard.
2. Select another extension/license entry.
3. Repeat activation, renewal, and status checks for each product.
